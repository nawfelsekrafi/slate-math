...
import { getPlatePluginType, SPEditor, useEventEditorId, useStoreEditorRef } from "@udecode/plate-core";
import { BaseEditor, Editor, Path, Range, Transforms } from "slate";
import { ReactEditor } from "slate-react";
import { ELEMENT_EQUATIONBOX } from "./EquationBoxElement";

export const getCurrentSelection = (editor:SPEditor, root?:boolean):Path | undefined => {
...
export const selectFirstBox = (editor: BaseEditor | undefined) => {
    var selection = getCurrentSelection(editor as SPEditor, true)
    if(selection){
        let textNode = findNode(editor as SPEditor, {at: selection, match: {type: getPlatePluginType(editor as SPEditor, ELEMENT_EQUATIONBOX)}})
        if(textNode){
            let target = Editor.range(editor as SPEditor, textNode[1])
            ReactEditor.focus(editor as ReactEditor)
            Transforms.setSelection(editor as SPEditor, target)
        }
    }
...
export const equationBoxOnKeyDown = (): KeyboardHandler => (editor:SPEditor) => (e) => {
        ...
        var selection = getCurrentSelection(editor)
          if (selection) {
            ...
            while (getNode(editor,sibling)){ //check if math equation node has any more boxes
              let textNode = findNode(editor, {at: sibling, match: {type: getPlatePluginType(editor, ELEMENT_EQUATIONBOX)}})
...
export const containsMath = (editor: BaseEditor | undefined, selection:Path ):boolean => {
    return someNode(editor as SPEditor, {at:selection, match: {type: getPlatePluginType(editor as SPEditor, ELEMENT_EQUATIONBOX),} })
}