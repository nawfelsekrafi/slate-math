...
export const equationBoxOnKeyDown = (): KeyboardHandler => (editor) => (e) => {
  if (e.key === 'Enter') {
    ...
    if (selection) {
      if (!containsMath(editor, selection) || comboboxStore.get.isOpen())
        //if not at math node of if combobox is open do nothing
        return
      let pathParent = Path.parent(selection) //get parent
      let sibling = Path.next(pathParent)
      while (getNode(editor, sibling)) {
        //check if math equation node has any more boxes
        let textNode = findNode(editor, {
          at: sibling,
          match: { type: getPluginType(editor, ELEMENT_EQUATIONBOX) },
        })
        if (textNode) {
          //if found another box, select it
    ...
    if (selection) {
      ...
      if (pathParent.at(pathParent.length - 1) !== 0) {
        //make sure sibling isnt negative
        ...
        while (getNode(editor, sibling)) {
          ...
          let textNode = findNode(editor, {
            at: sibling,
            match: {
              type: getPluginType(editor as PlateEditor, ELEMENT_EQUATIONBOX),
            },
          })
          if (textNode) {
            //if found another box, select it
